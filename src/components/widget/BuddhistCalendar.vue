<template>
    <span ref="container" :id="id" :class="containerClass">
        <input :ref="inputRef" v-if="!inline" type="text" role="combobox" :id="inputId"
            :class="['p-inputtext p-component', inputClass]" :style="inputStyle" :placeholder="placeholder"
            aria-autocomplete="none" aria-haspopup="dialog" :aria-expanded="overlayVisible" :aria-controls="panelId"
            :aria-labelledby="ariaLabelledby" :aria-label="ariaLabel" inputmode="none" @input="onInput" @focus="onFocus"
            @blur="onBlur" @keydown="onKeyDown" :readonly="!manualInput" :tabindex="0" v-bind="inputProps">
        <CalendarButton v-if="showIcon" :icon="icon" class="p-datepicker-trigger" :disabled="disabled"
            @click="onButtonClick" type="button" :aria-label="$primevue.config.locale.chooseDate" aria-haspopup="dialog"
            :aria-expanded="overlayVisible" :aria-controls="panelId" />
        <Portal :appendTo="appendTo" :disabled="inline">
            <transition name="p-connected-overlay" @enter="onOverlayEnter($event)" @after-enter="onOverlayEnterComplete"
                ...
                <div :ref="overlayRef" :id="panelId" :class="panelStyleClass" :style=panelStyle
                    v-if="inline || overlayVisible" :role="inline ? null : 'dialog'"
                    :aria-modal="inline ? null : 'true'" :aria-label="$primevue.config.locale.chooseDate"
                            ...
                            <div class="p-datepicker-group" v-for="(month, groupIndex) of months"
                                ...
                                <div class="p-datepicker-header">
                                    ...
                                    <button class="p-datepicker-prev p-link" v-show="groupIndex === 0"
                                        @click="onPrevButtonClick" type="button" @keydown="onContainerButtonKeydown"
                                    ...
                                    <div class="p-datepicker-title">
                                        <button type="button" @click="switchToMonthView"
                                            @keydown="onContainerButtonKeydown" v-if="currentView === 'date'"
                                        ...
                                        <button type="button" @click="switchToYearView"
                                            @keydown="onContainerButtonKeydown" v-if="currentView !== 'year'"
                                        ...
                                        <span class="p-datepicker-decade" v-if="currentView === 'year'">
                                            <slot name="decade" :years="yearPickerValues">
                                                {{ getBuddhistYear(yearPickerValues[0]) }} -
                                                {{ getBuddhistYear(yearPickerValues[yearPickerValues.length - 1]) }}
                                            </slot>
                                        </span>
                                    </div>
                                ...
                                <div class="p-datepicker-calendar-container" v-if="currentView === 'date'">
                                    <table class="p-datepicker-calendar" role="grid">
                                        ...
                                        <tbody>
                                            <tr v-for="(week, i) of month.dates"
                                                ...
                                                <td v-for="date of week" :aria-label="date.day"
                    ...
                    <div class="p-timepicker" v-if="(showTime || timeOnly) && currentView === 'date'"
                        ...
                        <div class="p-hour-picker">
                            <button class="p-link" :aria-label="$primevue.config.locale.nextHour"
                            ...
                            <button class="p-link" :aria-label="$primevue.config.locale.prevHour"
                                @mousedown="onTimePickerElementMouseDown($event, 0, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)" @keydown="onContainerButtonKeydown"
                                v-ripple @mouseleave="onTimePickerElementMouseLeave()"
                                @keydown.enter="onTimePickerElementMouseDown($event, 0, -1)"
                        ...
                        <div class="p-minute-picker">
                            <button class="p-link" :aria-label="$primevue.config.locale.nextMinute"
                                @mousedown="onTimePickerElementMouseDown($event, 1, 1)"
                                @mouseup="onTimePickerElementMouseUp($event)" @keydown="onContainerButtonKeydown"
                                v-ripple :disabled="disabled" @mouseleave="onTimePickerElementMouseLeave()"
                            ...
                            <button class="p-link" :aria-label="$primevue.config.locale.prevMinute"
                                @mousedown="onTimePickerElementMouseDown($event, 1, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)" @keydown="onContainerButtonKeydown"
                                v-ripple :disabled="disabled" @mouseleave="onTimePickerElementMouseLeave()"
                                @keydown.enter="onTimePickerElementMouseDown($event, 1, -1)"
                        ...
                        <div class="p-second-picker" v-if="showSeconds">
                            <button class="p-link" :aria-label="$primevue.config.locale.nextSecond"
                            ...
                            <button class="p-link" :aria-label="$primevue.config.locale.prevSecond"
                                @mousedown="onTimePickerElementMouseDown($event, 2, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)" @keydown="onContainerButtonKeydown"
                                v-ripple :disabled="disabled" @mouseleave="onTimePickerElementMouseLeave()"
                                @keydown.enter="onTimePickerElementMouseDown($event, 2, -1)"
                        ...
                        <div class="p-ampm-picker" v-if="hourFormat == '12'">
                            <button class="p-link" :aria-label="$primevue.config.locale.am" @click="toggleAMPM($event)"
                                type="button" v-ripple :disabled="disabled">
                                <span class="pi pi-chevron-up"></span>
                            </button>
                            <span>{{ pm ? 'PM' : 'AM' }}</span>
                            <button class="p-link" :aria-label="$primevue.config.locale.pm" @click="toggleAMPM($event)"
                                type="button" v-ripple :disabled="disabled">
                                <span class="pi pi-chevron-down"></span>
                            </button>
                        </div>
...
export default {
    ...
    methods: {
        ...
        selectDate(dateMeta) {
            let smlNewDate = new Date();
            let date = new Date(dateMeta.year, dateMeta.month, dateMeta.day, smlNewDate.getHours(), smlNewDate.getMinutes(), smlNewDate.getSeconds(), smlNewDate.getMilliseconds());
                    ...
                    if (literal) {
                        ...
                        switch (format.charAt(iFormat)) {
                            ...
                            case 'M':
                                ...
                                break;
                            case 'y':
                                output += lookAhead('y') ? this.getBuddhistYear(date.getFullYear()) : (this.getBuddhistYear(date.getFullYear()) % 100 < 10 ? '0' : '') + (this.getBuddhistYear(date.getFullYear()) % 100);
                                break;
                            case '@':
                                output += date.getTime();
                                break;
        ...
        getYear(month) {
            return this.currentView === 'month' ? this.getBuddhistYear(this.currentYear) : this.getBuddhistYear(month.year);
        },
