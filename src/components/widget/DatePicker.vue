                ...
                <div class="p-datepicker-header">
                  ...
                  <div class="p-datepicker-title">
                    ...
                    <button
                      ...
                      :disabled="switchViewButtonDisabled"
                    >
                      {{ buddhist ? getYear(month) + 543 : getYear(month) }}
                    </button>
                    <span
...
export default {
  ...
  methods: {
    ...
    isMonthSelected(month) {
      if (this.isComparable()) {
        let value = this.isRangeSelection()
          ? this.modelValue[0]
          : this.modelValue;
    ...
    updateCurrentTimeMeta(date) {
      ...
      this.currentMinute =
        Math.floor(date.getMinutes() / this.stepMinute) * this.stepMinute;
      this.currentSecond =
        Math.floor(date.getSeconds() / this.stepSecond) * this.stepSecond;
    },
    ...
    incrementMinute(event) {
      ...
      if (
        this.validateTime(this.currentHour, newMinute, this.currentSecond, true)
      ) {
    ...
    incrementSecond(event) {
      ...
      if (
        this.validateTime(this.currentHour, this.currentMinute, newSecond, true)
      ) {
    ...
    onKeyDown(event) {
      if (event.keyCode === 40 && this.overlay) {
        ...
        if (this.overlay) {
          DomHandler.getFocusableElements(this.overlay).forEach(
            (el) => (el.tabIndex = "-1")
          );
        }
