...
onMounted(async () => {
    ...
    storeApp.setPageTitle("แก้ไขข้อมูลสมุดรายวัน");
...
async function getDetail(id) {
    try {
        const res = await MasterdataService.getJournalBookById(id)
        if (res.success) {
            //console.log(res);
            if (res.success) {
...
async function onSave(data) {
    // console.log(data);
    try {
        const res = await MasterdataService.putJournalBook(data, data.guidfixed)
        //console.log(res);
        if (res.success) {
            toast.add({
                severity: "success",
    ...
    <AppLayout>
        <MainContentWarp>
            <div class="surface-ground px-2 py-2">
                <Form :form_model="form_model" v-on:save="onSave" v-on:back="goTo('bookList', '')" />
            </div>
        </MainContentWarp>
    </AppLayout>
</template>
